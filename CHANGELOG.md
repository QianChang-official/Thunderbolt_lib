...
- Branch metadata, README files, and dependency examples now describe the isolated `26.1.2` / JDK 25 environment and the alpha branch version strings.
...
- Local validation passed with `./gradlew.bat clean build --no-daemon` in the isolated `Thunderbolt_lib_neoforge_26.1.2` directory under JDK 25, producing `build/libs/Thunderbolt_lib-1.0.10-alpha.26.1.2neoforge.jar`.
