 ...
 */
public record CraftInput<K>(K key, long amount, boolean returned, long uses) {
    ...
    public static <K> CraftInput<K> returned(K key, long amount) {
        return new CraftInput<>(key, amount, true, INFINITE_USES);
    }
    ...
    public static <K> CraftInput<K> finiteUse(K key, long amount, long uses) {
        return new CraftInput<>(key, amount, true, uses);
    }
